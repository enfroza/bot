#!/usr/bin/env python3
"""
Ape De File Saver - Full Production Ready Bot with Force Subscribe + Try Again
"""
...
def get_user_settings(user_id: int) -> dict:
    ...
    if row:
        return dict(row)
    return {"user_id": user_id, "protect_content": False, "custom_caption": "", "custom_button_text": "", "custom_button_url": ""}
...
async def store_message(client: Client, message: Message) -> Dict:
    try:
        copied = await client.copy_message(chat_id=STORAGE_CHANNEL_ID, from_chat_id=message.chat.id, message_id=message.id)
...
async def deliver_share(client: Client, share: dict, chat_id: int):
    ...
    for item in content_list:
        try:
            await client.copy_message(chat_id=chat_id, from_chat_id=item["chat_id"], message_id=item["message_id"], protect_content=protect, caption=caption if caption else None, reply_markup=keyboard if (sent_count == len(content_list) - 1) else None)
            sent_count += 1
...
async def auto_store_single(client: Client, message: Message):
    ...
    try:
        ...
        code = create_share(owner_id=user_id, content_list=[ref], protect=protect, caption=settings.get("custom_caption", ""), button_text=settings.get("custom_button_text", ""), button_url=settings.get("custom_button_url", ""))
        link = f"https://t.me/{(await client.get_me()).username}?start={code}"
        protect_text = "🔒 Protected" if protect else "🔓 Standard"
...
async def genlink_cmd(client: Client, message: Message):
    ...
    if not message.reply_to_message:
        await message.reply("Please reply to a file/message with /genlink")
        return
    try:
